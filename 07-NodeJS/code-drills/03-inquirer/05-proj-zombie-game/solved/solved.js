...
//For inquirer prompt
const zombieChoices = ["zombie1", "zombie2", "zombie3", "zombie4", "zombie5"];

//For object lookup
const zombieIndex = ["zombie1", "zombie2", "zombie3", "zombie4", "zombie5"];
...
function playRound() {
  ...
  inquirer
    ...
    .then(function (game) {
      ...
      switch (game.gun) {
        case "Bare hands":
          playerDam = calcDamage(
            weapons.bareHands.attack[0],
            weapons.bareHands.attack[1]
          );
        ...
        case "Machete":
          playerDam = calcDamage(
            weapons.machete.attack[0],
            weapons.machete.attack[1]
          );
        ...
        case "Shotgun":
          playerDam = calcDamage(
            weapons.shotgun.attack[0],
            weapons.shotgun.attack[1]
          );
        ...
        case "Pistol":
          playerDam = calcDamage(
            weapons.pistol.attack[0],
            weapons.pistol.attack[1]
          );
        ...
        case "Machine Gun":
          playerDam = calcDamage(
            weapons.machineGun.attack[0],
            weapons.machineGun.attack[1]
          );
        ...
        case "Sniper":
          playerDam = calcDamage(
            weapons.sniper.attack[0],
            weapons.sniper.attack[1]
          );
...
function pickZombie(weapon) {
  inquirer
    ...
    .then(function (zombie) {
      ...
      for (let i = 0; i < zombieChoices.length; i++) {
        if (
          zombieVictims.length < weapon.reach &&
          zombieChoices[i] !== zombie.zombie
        ) {
      ...
      if (!playerDidDam) {
        console.log("\nShot on target");
        for (let i = 0; i < zombieVictims.length; i++) {
          console.log(
            "\nCongrats, you landed a shot for " +
              playerDam +
              " damage on " +
              zombieVictims[i] +
              "!"
      ...
      let zombieDamSum =
        calcDamage(zombies.zombie1.attack[0], [1]) +
        calcDamage(zombies.zombie2.attack[0], [1]) +
        calcDamage(zombies.zombie3.attack[0], [1]) +
        calcDamage(zombies.zombie4.attack[0], [1]) +
        calcDamage(zombies.zombie5.attack[0], [1]);
...
console.log(
  "Bare hands: Zombies don't feel punches the same way humans do. Only use these as a last resort \n uses: ∞ \n\n",
